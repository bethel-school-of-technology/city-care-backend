          ...
          if(token) {
                    authService.verifyUser(token).then(org => {
                              if(org) {
                                        ...
                                        res.status(400).json({ message: 'Unable to update this organization.'})
                              }  
                    })
          } else {
                    ...
                    authService.verifyUser(token).then(org => {
                              ...
                              if(org) {
                                        models.organizations.delete({
                                                  ...
                                                  if(result) {
                                                            res.status(200).json({ message: 'This organization has been removed!'})
                                                  }
                                        });
                              } else {
                                        res.status(400).json({ message: 'This organization can not be removed!' })
                              }
                    });
          } else {
                    res.status(500).json({ message: 'Whoops, something went wrong!'})
